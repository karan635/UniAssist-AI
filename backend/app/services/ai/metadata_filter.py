...
def _debug_enabled() -> bool:
    # Deliberately os.getenv() here, not app.core.config.settings: this
    # module is a pure text/metadata matcher and previously had no config
    # dependency. Settings has no default values, so importing it requires
    # a fully populated .env (GROQ_API_KEY, VECTOR_PATH, etc.) just to
    # check a debug flag, which would break this module in isolation
    # (e.g. under unit tests that don't set up the whole app config).
    return os.getenv("DEBUG", "").strip().lower() in {
        "1", "true", "yes", "on"
    }


# Same rationale as retriever.py: gate this module's existing per-request
# debug print() tracing behind DEBUG instead of always printing.
